@extends('master')
@section('title', $page->getTranslatedAttribute('title', 'locale', App::getlocale()))
        ...
        <div class="breadcrumbs-custom-inner">
            <div class="container breadcrumbs-custom-container">
                <div class="breadcrumbs-custom-main">
                    <h6 class="breadcrumbs-custom-subtitle title-decorated">@lang('app.about-us')</h6>
                    <h1 class="breadcrumbs-custom-title">{{$page->getTranslatedAttribute('title', 'locale', App::getlocale())}}</h1>
                </div>
                <ul class="breadcrumbs-custom-path">
                    <li><a href="/">@lang('app.home')</a></li>
                    <li><a href="{{route('post.index')}}">@lang('app.blog')</a></li>
                    <li class="active">{{$page->getTranslatedAttribute('title', 'locale', App::getlocale())}}</li>
                </ul>
            </div>
        </div>
    </section>
            ...
            <div class="row row-50">
                <div class="col-lg-8">
                    <article class="post-creative">
                        <h3 class="post-creative-title">{{$page->getTranslatedAttribute('title', 'locale', App::getlocale())}}</h3>
                        <ul class="post-creative-meta">
                            <li><span class="icon mdi mdi-calendar-clock"></span>
                                <time datetime="{{ \Carbon\Carbon::parse($page->updated_at)->format('d/m/Y')}}">{{ \Carbon\Carbon::parse($page->updated_at)->format('d/m/Y')}}</time>
                            </li>
                            <li><span class="icon mdi mdi-tag-multiple"></span><a href="{{route('about')}}">@lang('app.about-us')</a></li>
                        </ul>
                        <h4>{{$page->getTranslatedAttribute('excerpt', 'locale', App::getlocale())}}</h4><img src="/storage/{{$page->image}}" alt="" width="770"/>
                        {!! $page->getTranslatedAttribute('body', 'locale', App::getlocale()) !!}
